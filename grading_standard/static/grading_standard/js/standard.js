// Javascript for Grading Standard BLTI
...
$(document).ready(function () {
    ...
    var prependSavedScheme = function (scheme, which) {
        ...
        $('button', node).click(function () {
            var tpl = Handlebars.compile($('#remove-modal-tmpl').html()),
                modal_container;

            $('body').append(tpl({
                                      name: scheme.name,
                                      scheme_edit_url: window.grading_standard.blti.launch_presentation_return_url
                                                       + '/grading_standards',
                                      course_label: window.grading_standard.blti.context_label
                                 }));
    ...
    var loadGradingStandard = function () {
    ...
    var saveGradingStandard = function (name, scheme) {
        var tpl, modal_container;
        ...
        $('#save-valid-scheme', modal_container).click(function (e) {
            var name = $('#scheme-name', modal_container).val().trim(),
                proceed = true;
            ...
            if (name.length) {
                $.ajax({
                    type: 'GET',
                    url: 'api/v1/grading_standards/?name=' + encodeURIComponent(name),
                    async: false
                }).fail(function (xhr, textStatus, errorThrown) {
                    if (xhr.status != 404) {
                        ajaxFailureModal(xhr.responseText);
                ...
                modal_container.modal('hide');
                postGradingStandard(name, scheme);
            }
        });
    };

    postGradingStandard = function (name, scheme) {
        var request_data = {
            grading_standard: {
                name: name,
    ...
    var loadSaveSuccessMessage = function (standard) {
    ...
    var reportErrorCount = function (error_count) {
    ...
    var loadGradingCalculator = function (opts) {
        ...
        container_node.on("renderedGradeConversion", function (e) {
        ...
        calc.on("cancelGradeConversion", function (e, d) {
            e.preventDefault();
            loadGradingStandard();
        });
    };
